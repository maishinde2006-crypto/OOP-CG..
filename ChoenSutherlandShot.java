...
public class CohenSutherlandShort extends JPanel {
    ...
    int code(int x, int y) {
        int c=INSIDE;
        if(x<x_min) c|=LEFT; else if(x>x_max) c|=RIGHT;
        if(y<y_min) c|=BOTTOM; else if(y>y_max) c|=TOP;
        return c;
    }
    ...
    void clip(Graphics g, int x1,int y1,int x2,int y2) {
        ...
        while(true){
            if((c1|c2)==0){ g.setColor(Color.RED); g.drawLine(x1,y1,x2,y2); break; }
            else if((c1&c2)!=0) break;
            else{
                int c=(c1!=0)?c1:c2, x=0,y=0;
                if((c&TOP)!=0){ x=x1+(x2-x1)*(y_max-y1)/(y2-y1); y=y_max; }
                else if((c&BOTTOM)!=0){ x=x1+(x2-x1)*(y_min-y1)/(y2-y1); y=y_min; }
                else if((c&RIGHT)!=0){ y=y1+(y2-y1)*(x_max-x1)/(x2-x1); x=x_max; }
                else if((c&LEFT)!=0){ y=y1+(y2-y1)*(x_min-x1)/(x2-x1); x=x_min; }
